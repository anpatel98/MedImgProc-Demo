...
MappingFile="${StudyFolder}/prisma_Diffusion_mapping.txt"
...
### BEGIN preparing data ###
  ...
  # Change --options="addImageType:10|addJSONInfo:DwellTime,TR,PEDirection,ReadoutDirection" to 
...
### BEGIN pipeline_specific ###
  ...
  # Ideally, the json parser 'jq' could be used, but not available inside qunex, so we'll use
  # the following one-liner using in-built UNIX commands (assuming that json is formatted similarly throughout study)
